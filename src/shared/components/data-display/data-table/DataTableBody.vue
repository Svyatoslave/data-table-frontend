<template>
  <VLoadingOverlay has-overlay fixed :visible="loading" />
...
export interface DataTableBody<T extends RowData = RowData> {
  rows: Optional<T[]>;
  loading: boolean;
...
const props = withDefaults(defineProps<DataTableBody>(), {
  isDisabledRow: () => false,
  getTooltipRow: () => null,
});
